# -*- coding: utf-8 -*-
"""
Created on Thu Sep  4 06:54:40 2025
...
# -*- coding: utf-8 -*-
"""
Mould 900 x 420 mm • Control in 3 Zones (5+5+5 heaters)
- Rescales CSV coordinates to 900x420
- Maps points to the 15 real longitudinal zones (via centres)
- Asks for power for Zone 1, Zone 2, Zone 3 (each is applied to the 5 heaters of the zone)
- Performs automatic per-zone adjustment (K * relative error) with limits [P_MIN, P_MAX]
- Generates plots: 3D, 2D, 3-zone bar chart and 2D overlay of the bands
"""
...
# Proportional controller (gain) and physical power limits
K = 1.5
P_MIN = 10
P_MAX = 150
...
# --- Fixed target temperatures (°C) per macro-zone — edit here ---
# Z1: heaters 1–5 | Z2: 6–10 | Z3: 11–15
T_SET_1 = 160
T_SET_2 = 140
T_SET_3 = 120

# Real centres (mm) of the 15 heaters
CENTROS = [
...
# Strip whitespace only for string columns (modern Pandas, no applymap warning)
...
# ==========================
# 15 REAL ZONES (limits by midpoints between CENTRES)
# ==========================
limits = [0.0]
for i in range(1, len(CENTROS)):
    limits.append(0.5 * (CENTROS[i-1] + CENTROS[i]))
limits.append(MOLD_X)
x_edges = np.array(limits, dtype=float)  # 16 edges → 15 zones
...
# ==========================
# FIXED BORDER (ignore periphery without part)
# ==========================
# simple validations for borders
if BX_esq + BX_dir >= MOLD_X - 1e-6:
    raise ValueError("Sum of borders in X exceeds the mould length.")
if BZ_inf + BZ_sup >= MOLD_Z - 1e-6:
    raise ValueError("Sum of borders in Z exceeds the mould width.")
...
# Means per macro-zone WITH border applied
medias_macro = df_util.groupby('Zona_Macro')['Value (Celsius)'] \
                      .mean().reindex([1, 2, 3])
print("\nMean temperature per Macro-Zone (with border applied):")
for zmac in [1, 2, 3]:
    val = float(medias_macro.loc[zmac]) if pd.notna(medias_macro.loc[zmac]) else np.nan
    print(f"  Zone {zmac}: {val:.2f} °C")

# Function to draw the useful area
...
# ==========================
# BASE PLOTS (3D and 2D), with useful area
# ==========================
fig = plt.figure(figsize=(10, 8))
ax3d = fig.add_subplot(111, projection='3d')
sc = ax3d.scatter(x, z, T, c=T, cmap='plasma')
ax3d.set_xlabel('X (mm)'); ax3d.set_ylabel('Z (mm)'); ax3d.set_zlabel('T (°C)')
ax3d.set_title('Temperature Distribution — 3D')
plt.colorbar(sc, ax=ax3d, pad=0.1, label='Temperature (°C)')
...
# ==========================
# SUBDIVISION 15 × 4 (diagnostics) — shows full mould + useful area
# ==========================
z_edges = np.linspace(0.0, MOLD_Z, 5)   # 5 edges → 4 rows
...
ax.set_title('Subdivision 15×4: Mean Temperature and ΔT per Subzone (60 zones)')
...
# ==========================
# INPUTS: INITIAL POWER per macro-zone (kept)
# ==========================
def pedir_float(txt):
    while True:
        try:
...
print("\nEnter the power (W) for each macro-zone (applied to the 5 heaters of that zone):")
PZ1 = pedir_float("  Zone 1: ")
PZ2 = pedir_float("  Zone 2: ")
PZ3 = pedir_float("  Zone 3: ")

# ==========================
# ADJUSTMENT (multiplicative proportional) WITH means of the useful area
# ==========================
...
print("\n=== Adjustment per Macro-Zone (with border applied) ===")
...
# Adjusted vector per heater (repeats the new power of the macro-zone)
...
# ==========================
# PLOT: POWER PER ZONE (BEFORE vs AFTER)
# ==========================
...
larg = 0.35
ax1.bar(zonas - larg/2, pot_ini, width=larg, label='Initial Power (W)', alpha=0.9)
ax1.bar(zonas + larg/2, pot_new, width=larg, label='New Power (W)', alpha=0.9)
ax1.set_xticks(zonas)
ax1.set_xlabel('Macro-Zone')
ax1.set_ylabel('Power (W)')
ax1.set_title('Power Adjustment per Zone (3 zones)')
ax1.legend()
ax1.grid(True, axis='y', linestyle='--', alpha=0.5)

ax2 = ax1.twinx()
ax2.plot(zonas, ajuste_df['Erro (°C)'], linestyle='-', marker='s', label='Thermal error (°C)')
ax2.set_ylabel('Thermal error (°C)')
ax2.tick_params(axis='y')
plt.tight_layout(); plt.show()

# ==========================
# 2D OVERLAY: limits of the 15 zones + bands of 3 macro-zones + useful area
# ==========================
plt.figure(figsize=(10, 8))
cf = plt.contourf(XI, ZI, TI, levels=100, cmap='plasma')
for e in x_edges:
...
plt.title('2D Map with Macro-Zones and Adjusted Power')
plt.colorbar(cf, label='Temperature (°C)')
plt.legend(loc='lower center', ncol=3, frameon=False)
plt.tight_layout(); plt.show()

# ==========================
# PLOT PER HEATER: 15 BARS (with real position in mm)
# ==========================
fig, ax = plt.subplots(figsize=(12, 5.8))
idx = np.arange(1, 16, dtype=int)

# backgrounds by macro-zone
ax.axvspan(0.5, 5.5,  alpha=0.08)
ax.axvspan(5.5, 10.5, alpha=0.08)
ax.axvspan(10.5, 15.5, alpha=0.08)

bars = ax.bar(idx, pot_ajustada_15, alpha=0.95, label='Adjusted Power (W)')
ax.set_xlabel('Heater (R)  |  Position along X (mm)')
ax.set_ylabel('Power (W)')
ax.set_title('Power per Heater (after adjustment) — with real position')

ax.set_xticks(idx)
ax.set_xticklabels([f"R{i}\n{CENTROS[i-1]:.1f} mm" for i in idx], rotation=0)

# thin lines between bars (equivalent to real half-distances)
